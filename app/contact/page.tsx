          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white rounded-xl p-6 text-center shadow-lg">
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-2">{c(contentData, 'contact.info.phone_title', 'お電話でのお問い合わせ')}</h3>
                <div className="text-2xl font-bold text-blue-600 mb-1">{c(contentData, 'contact.info.phone_number', '096-329-2426')}</div>
                <div className="text-sm text-gray-600">{c(contentData, 'contact.info.phone_hours', '平日 9:00-18:00')}</div>
              </div>
              <div className="bg-white rounded-xl p-6 text-center shadow-lg">
                <Mail className="text-blue-600 mx-auto mb-4" size={32} />
                <h3 className="text-lg font-bold text-gray-900 mb-2">{c(contentData, 'contact.info.email_title', 'メールでのお問い合わせ')}</h3>
                <div className="text-lg text-blue-600 mb-1">{c(contentData, 'contact.info.email_address', 'info@precision-lab.jp')}</div>
                <div className="text-sm text-gray-600">{c(contentData, 'contact.info.email_note', '24時間受付')}</div>
              </div>
              <div className="bg-white rounded-xl p-6 text-center shadow-lg">
                <CreditCard className="text-blue-600 mx-auto mb-4" size={32} />
                <h3 className="text-lg font-bold text-gray-900 mb-2">{c(contentData, 'contact.info.payment_title', '決済方法について')}</h3>
                <div className="text-sm text-gray-600 whitespace-pre-line">{c(contentData, 'contact.info.payment_text', 'クレジットカード決済\n銀行振込に対応')}</div>
              </div>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-lg">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <h4 className="font-bold text-gray-900 mb-2">{c(contentData, 'contact.info.notice_title', '重要なお知らせ')}</h4>
                  <p className="text-gray-600 text-sm">{c(contentData, 'contact.info.notice_text', '料金のお見積もり、決済方法につきましても、お問い合わせいただいた後、担当者より個別にご連絡させていただきます。製作内容や納期についてもお気軽にご相談ください。')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold text-gray-900 mb-6">{c(contentData, 'contact.form.title', 'お問い合わせフォーム')}</h2>
              <p className="text-xl text-gray-600">{c(contentData, 'contact.form.subtitle', '下記フォームに必要事項をご入力の上、送信してください')}</p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <label htmlFor="privacyAgreed" className="text-sm text-gray-700">
                    <span className="font-semibold">{c(contentData, 'contact.form.label_privacy', 'プライバシーポリシーに同意する *')}</span>
                    <br />
                ...
                <div className="text-center pt-6">
                  ...
                  >
                    <Send size={20} />
                    <span>{c(contentData, 'contact.form.btn_submit', 'お問い合わせを送信')}</span>
                  </button>
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-gray-900 mb-6">{c(contentData, 'contact.faq.title', 'よくある質問')}</h2>
            <p className="text-xl text-gray-600">{c(contentData, 'contact.faq.subtitle', 'お問い合わせ前に、こちらもご確認ください')}</p>
          </div>
